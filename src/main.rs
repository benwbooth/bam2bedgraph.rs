...
fn cigar2exons(exons: &mut Vec<(i32, i32)>, cigar: &Vec<Cigar>, pos: i32) {
    let mut pos = pos;
    for op in cigar {
        match op {
            &Cigar::Match(length) => {
...
fn analyze_bam(options: &Options,
    ...
    let bam = match Reader::new(&options.bamfile) {
        Ok(reader) => { reader },
        Err(_) => { panic!("Error: BGZFError") }
    };
    ...
    while bam.read(&mut read).is_ok() {
        ...
        if options.uniq {
            ...
            if hits.is_none() || hits.unwrap().integer() != 1 {
                continue;
            }
        }
        ...
        if !options.split_exons && exons.len() > 0 {
            exons = vec![(exons.get(0).unwrap().0,
                          exons.get(exons.len() - 1).unwrap().1)];
        }
        let read_number = if read.is_second_in_pair() {
            2
        } else {
            1
        };
        ...
        let strand = if xs.is_some() {
            let strand = str::from_utf8(xs.unwrap().string()).unwrap();
            strand
        } else if read_number == 1 {
            if split_strand.as_bytes()[0] == 'r' as u8 {
                if read.is_reverse() {
                    "+"
                } else {
                    "-"
                }
            } else if split_strand.as_bytes()[0] == 's' as u8 {
                if read.is_reverse() {
                    "-"
                } else {
                    "+"
            ...
            if split_strand.as_bytes()[1] == 's' as u8 {
                if read.is_reverse() {
                    "-"
                } else {
                    "+"
                }
            } else if split_strand.as_bytes()[1] == 'r' as u8 {
                if read.is_reverse() {
                    "+"
                } else {
                    "-"
        ...
        let read_num = if options.split_read {
            read_number
        } else {
            0
        };
                ...
                if intervals.is_some() {
                    ...
                    if intervals.contains_key(&refs[lastchr as usize].1) {
                        ...
                        for interval in overlapping_annot {
                            ...
                            let strandtype = if read.is_reverse() ==
                                                (interval.value == '-' as u8) {
                                's'
                            } else {
                                'r'
                            };
                            if read_number == 1 {
                                *autostrand_totals.get_mut(&strandtype).unwrap() +=
                                    overlap_length as i64
                            } else if read_number == 2 {
                                *autostrand_totals2.get_mut(&strandtype).unwrap() +=
                                    overlap_length as i64
                            }
                        }
                    }
    ...
    if !autostrand_pass && options.bigwig {
        ...
        for fh in &fhs {
            ...
            let sorted_bedgraph = Regex::new(r"\.bedgraph$")
                                      .unwrap()
                                      .replace(fh.0, ".sorted.bedgraph");
            for command in vec![Command::new("sort")
                ...
                if !exit_code.success() {
                    if exit_code.code().is_some() {
                        panic!("Nonzero exit code {} returned from command: {:?}",
...
fn main() {
    ...
    let mut options = Options { ..Options::default() };
    {
        ...
        ap.refer(&mut options.split_strand)
          .add_option(&["--strand"],
                      Store,
                      "Split output bedgraph by strand: Possible values: u s r uu us ur su ss sr \
                       ru rs rr, first char is read1, second is read2, u=unstranded, s=stranded, \
                       r=reverse")
          .metavar("[TYPE]");
    ...
    if !Regex::new(r"^[usr][usr]$").unwrap().is_match(&options.split_strand) {
        panic!("Invalid value for split_strand: \"{}\": values must be one of: u s r uu us ur su \
                ss sr ru rs rr",
    ...
    if !options.autostrand.is_empty() {
        if !Path::new(&options.autostrand).exists() {
            panic!("Autostrand Bam file {} could not be found!",
                   &options.autostrand);
        }
        let bam = match rust_htslib::bam::Reader::new(&options.autostrand) {
            Ok(r) => r,
            Err(_) => panic!("BGZFError!")
        };
        let ref header = bam.header;
        ...
        while bam.read(&mut read).is_ok() {
            ...
            if exons.len() > 0 {
                interval_lists.get_mut(&chr)
                              .unwrap()
                              .push(intervaltree::Interval::new(read.pos() + 1,
                                                                exons[exons.len() - 1].1,
                                                                if read.is_reverse() {
                                                                    '-' as u8
                                                                } else {
        ...
        for (chr, list) in &interval_lists {
            ...
            intervals.as_mut()
                     .unwrap()
                     .insert(chr.clone(), intervaltree::IntervalTree::new_from(list));
        }
    }
